...
exec_graph_enable_brgconv = ''
exec_graph_disable_brgconv = ''
args = None
...
def show_compare_result(log_file_enable_brgconv, log_file_disable_brgconv, all_dict):
    ...
    for name in all_names:
        ...
        brgconv_info = [layer0, exectype0, layout0, time0]
        jit_info = [layer1, exectype1, layout1, time1]
        ...
        if args.node_type in info0 or args.node_type in info1:
            ...
            if args.show_verbose:
                ...
                brgconv_info += [verbose0, benchdnn0]
                jit_info += [verbose1, benchdnn1]
        layerid += 1
        dict_data = JsonData()
        layer_dict = dict_data.layer(layer_info, brgconv_info, jit_info, time_diff)
...
def get_exec_info(pc_by_node, name):
    ...
    if v0:
        ...
        layout = find_layout(exec_graph_enable_brgconv, name)
        ...
        if my_verbose_converter:
            ...
            if output != None:
                for key, value in output.items():
                    benchdnn = f"./benchdnn --fix-times-per-prb=1000 --mode=p {value}"
...
# brgconv_info = [node_type, exec_type, layout, time, onednn_cmd, benchdnn_cmd]
# jit_info = [node_type, exec_type, layout, time, onednn_cmd, benchdnn_cmd]
...
class JsonData:
    ...
    def layer(self, layer_info, brgconv_info, jit_info, time_diff):
        layer_id = "layer" + str(layer_info[0])
        self.jsonData[layer_id] = {}
        self.jsonData[layer_id]["layer_name"] = layer_info[1]
        self.jsonData[layer_id]["brgconv"] = self.exec_node(brgconv_info)
        self.jsonData[layer_id]["jit"] = self.exec_node(jit_info)
...
if __name__ == "__main__":
    ...
    parser.add_argument("log_file_enable_brgconv", nargs="?")
    parser.add_argument("log_file_disable_brgconv", nargs="?")
    parser.add_argument("exec_graph_enable_brgconv", nargs="?")
    parser.add_argument("exec_graph_disable_brgconv", nargs="?")
    ...
    data_cmp = show_compare_result(
        args.log_file_enable_brgconv or './enable_brgconv/pc_enable_brgconv.txt',
        args.log_file_disable_brgconv or './disable_brgconv/pc_disable_brgconv.txt',
        all_dict)
    write_json_data(data_cmp, args.output_file or './output', args.model)
